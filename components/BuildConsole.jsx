...
import { buildViteProjectFiles } from "../lib/deployWrap";
...
export default function BuildConsole({
  ...
  function handleImported({ files: importedFiles, analysis, truncated, sourceLabel }) {
    // Merge instead of replace — so importing a second repo/zip adds
    // alongside the first (prefixed by source) rather than wiping it out.
    setFiles((prev) => {
      const alreadyHasProject = Object.keys(prev).length > 1 || messages.length > 1;
      if (!alreadyHasProject) return importedFiles;
      const prefix = `/${sourceLabel || "imported"}`;
      const prefixed = {};
      for (const [p, content] of Object.entries(importedFiles)) {
        prefixed[`${prefix}${p}`] = content;
      }
    ...
    parts.push('\nAgar pehle se koi project khula tha, ye uske saath jud gaya hai (alag folder mein). Chat mein bata "in dono se ye feature nikaal ke combine karo" jaisa jo bhi karna hai.');
  ...
  function handleRestore(version) {
    ...
    setMessages((prev) => [
      ...
      {
        role: "assistant",
        text: `Version restore ho gaya (${new Date(version.created_at).toLocaleString()} wala). Abhi tak save nahi hua hai — "Save" dabao pakka karne ke liye.`,
      },
    ]);
  }

  async function callGenerate(promptMessages, image) {
    const res = await fetch("/api/generate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ messages: promptMessages, currentFiles: files, image }),
    });
    if (!res.ok) {
  ...
  async function handleSend() {
    ...
    if ((!prompt && !pendingImage) || loading) return;
    setInput("");
    const imageToSend = pendingImage;
    setPendingImage(null);
    errorFixAttempts.current = 0; // manual message resets the auto-fix budget
    const nextMessages = [
      ...messages,
      { role: "user", text: prompt || "Is image jaisa bana do", hasImage: !!imageToSend },
    ];
  ...
  const errorFixAttempts = useRef(0);
  const MAX_AUTO_FIX_ATTEMPTS = 2;

  async function handlePreviewError(errorText) {
    if (loading) return;
    if (errorFixAttempts.current >= MAX_AUTO_FIX_ATTEMPTS) return;
    errorFixAttempts.current += 1;
    ...
    setMessages((prev) => [
      ...
      {
        role: "assistant",
        text: `Preview mein error mila, khud fix karne ki koshish kar raha hoon (attempt ${errorFixAttempts.current}/${MAX_AUTO_FIX_ATTEMPTS})...`,
      },
    ]);
    ...
    try {
      const syntheticMessage = {
        role: "user",
        content: `AUTOMATIC ERROR FIX REQUEST — this is not something the person typed, it's a live error from the running preview. Fix it.\n\nError:\n${errorText}`,
      };
      const result = await callGenerate([
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", text: `Auto-fix nahi ho paya: ${e.message}. Khud bata kya galat hai.` },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  async function handleDownload() {
    const zip = new JSZip();
    const projectFiles =
      runtime === "browser" ? buildViteProjectFiles(files, dependencies) : files;
    ...
    a.href = url;
    a.download = `${(projectName || "forge-app").replace(/[^a-z0-9-]/gi, "-")}.zip`;
    a.click();
  ...
  return (
    <div className="grid h-screen grid-rows-[auto_1fr] bg-ink">
      ...
      <div className="flex flex-1 flex-col overflow-hidden md:grid md:grid-cols-[380px_1fr]">
        <div className="flex h-1/2 flex-col overflow-hidden border-r border-line bg-panel md:h-full">
          ...
          <div className="border-t border-line p-3">
            {pendingImage && (
              <div className="mb-2 flex items-center gap-2">
                <img src={pendingImage} alt="attached" className="h-12 w-12 rounded-md object-cover" />
                <span className="text-xs text-mist">Image attached — isi jaisa banega</span>
                <button
                  onClick={() => setPendingImage(null)}
                  className="ml-auto text-xs text-mist hover:text-bone"
                >
                  Hatao
                </button>
              </div>
            ...
            <div className="mt-2 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <label className="cursor-pointer rounded-md border border-line px-2.5 py-1.5 text-xs text-mist hover:text-bone">
                  📷
                  <input
                    type="file"
                    accept="image/*"
        ...
        <div className="h-1/2 overflow-hidden md:h-full">
          {runtime === "node" ? (
            <CodePanel files={files} runtime={runtime} onError={handlePreviewError} />
          ) : (
            <PreviewPanel files={files} dependencies={dependencies} onError={handlePreviewError} />
          )}
        </div>
      </div>
